...
sys.path.append('/root/np_app_VOTS2023/external/AR/pytracking/VOTS2023/sam_hq/') 
from lib.test.tracker.mixformer_convmae_online_delete_bbox_sam import MixFormerOnline
...
class MIXFORMER(object):
    ...
    def track(self, img_RGB, i, other_obj_bboxes, escape):
        '''TRACK'''
        '''base tracker'''
        # outputs = self.tracker.track(img_RGB, i, escape)
        outputs = self.tracker.track(img_RGB, i,other_obj_bboxes=other_obj_bboxes, escape=escape)
...
def main(vis=False):
    ...
    sam_checkpoint = "/root/np_app_VOTS2023/external/AR/pytracking/VOTS2023/sam_hq_vit_h.pth"
    model_type = "vit_h"
    ...
    for i, object in enumerate(objects):
        ...
        init_info = tracker.initialize(image, mask, i)
        trackers.append(tracker)
        # initialize obect bboxes
        object_id_bbox[i] = init_info['init_bbox']
        print(i,init_info['init_bbox'])
    ...
    while True:
        ...
        ### tracking
        # outputs = [t.track(image, i, escape=escapes[i]) for i, t in enumerate(trackers)]
        ...
        for i, t in enumerate(trackers):
            b, s, e = t.track(image, i,other_obj_bboxes=object_id_bbox ,escape=escapes[i])
            bbox.append(b)
            score.append(s)
            escapes[i] = e
            if e == True:
                object_id_bbox[i] = [-1,-1,-1,-1]
            else:       
                object_id_bbox[i] = b
