...
from lib.test.tracker.mixformer_convmae_online_shuffle_feature import MixFormerOnline
...
# 
    ...
    def track(self, img_RGB):
        ...
        '''Step2: Mask report'''
        if pred_score < 0.1:
            return np.zeros(img_RGB.shape[:2]).astype(np.uint8), pred_bbox, pred_bbox, pred_score
        ...
        return final_mask, pred_bbox, AR_bbox, pred_score
...
def main(vis=False):
    ...
    tracker_param = "mixformer_convmae_base_online.pth.tar"
    ...
    while True:
        ...
        score = list(map(lambda x: x[3], outputs))
